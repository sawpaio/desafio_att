...
def gravar(saida):
    ...
    arq = csv.writer(open(path + '/lucasSampaio/crawler_dolar/dolar_{}.csv'.format(time), '+a'), delimiter = ',')
	#escrita da linha
    if os.stat(path + '/lucasSampaio/crawler_dolar/dolar_{}.csv'.format(time)).st_size == 0:
